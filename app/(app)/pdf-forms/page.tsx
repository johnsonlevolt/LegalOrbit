...
export default async function PdfFormsPage() {
  const [templates, outputs] = await Promise.all([getPdfFormTemplates(), getPdfFormOutputs()])
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          既存の正式様式PDFに、案件データを座標指定で転記するためのテンプレートを管理します。
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 text-sm text-muted-foreground md:grid-cols-3">
          <div className="rounded-md border bg-white p-3">
            ...
            <p className="mt-1">国交省・自治体などの様式PDFをアップロードします。</p>
          </div>
          <div className="rounded-md border bg-white p-3">
            <p className="font-medium text-foreground">2. 転記位置を指定</p>
            <p className="mt-1">x/y座標と source をJSONで設定します。source が案件データの参照先です。</p>
          </div>
          <div className="rounded-md border bg-white p-3">
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            <TableHeader><TableRow><TableHead>帳票名</TableHead><TableHead>業務種別</TableHead><TableHead>項目数</TableHead></TableRow></TableHeader>
            <TableBody>
              {templates.length === 0 ? (
                <TableRow><TableCell colSpan={3} className="py-8 text-center text-muted-foreground">PDF帳票テンプレートはまだありません</TableCell></TableRow>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            <TableHeader><TableRow><TableHead>日時</TableHead><TableHead>帳票</TableHead><TableHead>保存先</TableHead></TableRow></TableHeader>
            <TableBody>
              {outputs.length === 0 ? (
                <TableRow><TableCell colSpan={3} className="py-8 text-center text-muted-foreground">出力履歴はまだありません</TableCell></TableRow>
              ) : outputs.map(output => (
